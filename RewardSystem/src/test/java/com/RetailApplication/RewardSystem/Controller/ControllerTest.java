...
import com.RetailApplication.RewardSystem.Dto.BulkSubTransaction;
...
class ControllerTest {
    ...
    void testGetCustomerRewards() throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders.get("/api/reward/customer/{customerId}", customerId)
                        .param("months", "3"))
    ...
    void testHandleTransaction() throws Exception {
        ...
        doNothing().when(rewardService).handleTransaction(transactionRequest.getCustomerId(),transactionRequest.getAmount(),LocalDateTime.now());

        mockMvc.perform(MockMvcRequestBuilders.post("/api/transaction/createTransaction")
    ...
    void testHandleBulkTransaction() throws Exception {
        ...
        bulkTransactionRequest.setTransactions(Arrays.asList(
        ...
        when(rewardService.handleBulkTransactions(eq(customerId), any())).thenReturn(bulkTransactionRequest);

        mockMvc.perform(MockMvcRequestBuilders.post("/api/transaction/createBulkTransaction")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"customerId\":\"" + customerId + "\", \"transactions\":[{\"amount\":120, \"transactionDate\":\"" + LocalDate.now() + "\"}, {\"amount\":80, \"transactionDate\":\"" + LocalDate.now().minusDays(1) + "\"}]}"))
                .andExpect(status().isOk())
                .andExpect(content().json("{\"customerId\":\"" + customerId + "\", \"transactions\":[{\"amount\":120, \"transactionDate\":\"" + LocalDate.now() + "\"}, {\"amount\":80, \"transactionDate\":\"" + LocalDate.now().minusDays(1) + "\"}]}"));
    }
    ...
    void testHandleBulkTransaction_InvalidRequest() throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders.post("/api/transaction/createBulkTransaction")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"transactions\":[{\"amount\":120, \"transactionDate\":\"" + LocalDate.now() + "\"}]}"))
                .andExpect(status().isBadRequest());
